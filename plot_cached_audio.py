                ...
                try:
                    ...
                    if isinstance(array, np.ndarray) and array.ndim == 1 and array.size > 0:
                        ...
                        if os.path.exists(filename):
                            ...
                            #     array, sample_rate
                            # )
                            fft_frequency, fft_confidence = get_pitch_fft_interpolated(
                                array, sample_rate
                            )
                            ...
                            # Filter to i                       # crepe_frequency, crepe_confidence = get_pitch_crepe(
                            #     array, sample_rate
                            # )nclude only positive frequencies up to 10000 Hz
                            ...
                            if fft_frequency <= 5000:
                                ax2.axvline(
                                    x=fft_frequency, color="g", linestyle="--"
                                )
                                ax2.text(
                                    fft_frequency,
                                    max(positive_magnitudes) / 2,
                                    f"{fft_frequency:.2f} Hz\nConf: {1:.2f}",
                                    color="g",
                                    ha="center",
                                )
